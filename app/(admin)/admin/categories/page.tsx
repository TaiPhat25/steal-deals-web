...
import { createCategory, deleteCategory, listCategories, updateCategory } from "@/lib/api/store";
...
export default function AdminCategories() {
  ...
  async function saveCategory(event: FormEvent<HTMLFormElement>) {
    ...
    if (editing === "new") {
      if (!accessToken) {
        setToast("Sign in as an admin before creating a category.");
        return;
      }
      setSubmitting(true);
      try {
        const category = await createCategory(accessToken, { name, slug, iconUrl });
        setCategories((items) => [category, ...items]);
        setToast(`${name} was created.`);
      } catch (caught) {
        setToast(caught instanceof Error ? caught.message : "Unable to create this category.");
        return;
      } finally {
        setSubmitting(false);
      }
      ...
      if (demoReason) {
        ...
        if (!accessToken) {
          setToast("Sign in as an admin before updating a category.");
          return;
        }
        setSubmitting(true);
        try {
          const category = await updateCategory(accessToken, editing.id, { name, slug, iconUrl, isActive });
          setCategories((items) => items.map((item) => item.id === editing.id ? category : item));
        } catch (caught) {
          setToast(caught instanceof Error ? caught.message : "Unable to update this category.");
          return;
        } finally {
          setSubmitting(false);
        }
  ...
  async function confirmDelete() {
    ...
    if (!demoReason) {
      if (!accessToken) {
        setToast("Sign in as an admin before deleting categories.");
        return;
      }
      setDeletingBusy(true);
      try {
        const results = await Promise.allSettled(ids.map((id) => Promise.resolve().then(() => deleteCategory(accessToken, id))));
        const failed = results.find((result) => result.status === "rejected");
        if (failed?.status === "rejected") {
          const refreshed = await Promise.resolve().then(listCategories).catch(() => null);
          if (refreshed) setCategories(refreshed);
          setToast(failed.reason instanceof Error ? failed.reason.message : "Unable to delete the selected categories.");
          return;
        }
      } finally {
  ...
  return (
    <>
      ...
      <DashboardCard className="w-full overflow-hidden">
        ...
        {loading ? <div className="border-t border-gray-500/20 px-4 py-14 text-center text-sm text-light-secondary-text" role="status">Loading categories…</div> : <>

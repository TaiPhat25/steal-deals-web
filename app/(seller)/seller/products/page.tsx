        ...
        <div className="overflow-x-auto border-t border-gray-500/20">
          <table className="w-full text-sm">
            ...
            <tbody>{rows.map((product) => <tr key={product.id} className="border-t border-gray-500/20 hover:bg-gray-50/50">
              ...
              <td className="p-3"><div className="flex items-center gap-3"><span className="size-11 overflow-hidden rounded-xl"><ProductImage alt="" src={product.imageUrl ?? (product as { ImageURL?: string }).ImageURL} /></span><div><strong className="block">{product.name}</strong><span className="text-xs text-light-secondary-text">#{product.id.slice(0, 8)}</span></div></div></td>

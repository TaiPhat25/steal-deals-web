...
export default function SellerDashboard() {
  ...
  const loading = settingsLoading || productsLoading || ordersLoading;
  const demoReason = settingsDemoReason || productsDemoReason || ordersDemoReason;
  ...
  async function confirmOrder(id: string) {
    ...
    try {
      if (ordersDemoReason) {
        ...
        const updated = await updateOrderStatus(accessToken, id, "Confirmed");
        setOrders((items) => items.map((item) => item.id === id ? updated : item));
      }
      setToast(`Order #${id.slice(0, 8)} confirmed.`);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Unable to confirm this order.");
    } finally {
      setUpdatingOrderId("");
    }
  }

...
export default function SellerOrders() {
  ...
  const stats = STATUSES.map((item) => ({ label: labels[item], value: orders.filter((order) => order.status === item).length }));
  ...
  }), [createdDate, orders, search, status]);
    ...
    <div className="space-y-6">
      ...
      <DashboardCard className="w-full overflow-hidden">
        <div className="p-4 sm:p-6">
          <div className="flex items-center justify-between gap-4"><div><p className="mb-1 text-xs font-semibold uppercase tracking-wide text-primary">Store orders</p><h1 className="text-xl font-bold">Orders</h1></div><DashboardButton onClick={exportCsv}>Export CSV</DashboardButton></div>
        ...
        <div className="overflow-x-auto border-t border-gray-500/20">
          ...
          {rows.length === 0 && <div className="px-4 py-14 text-center text-sm text-light-secondary-text">No orders match these filters.</div>}
        </div>

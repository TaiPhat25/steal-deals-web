...
try {
  ...
  await listMyCategorySuggestions("seller-token");
  const mySuggestionsReq = requests[12];
  assert.equal(mySuggestionsReq.url, "http://store.test/api/category-suggestions/me");
  assert.equal(mySuggestionsReq.options.method, "GET");
  assert.equal(new Headers(mySuggestionsReq.options.headers).get("Authorization"), "Bearer seller-token");

  await createCategorySuggestion("seller-token", { suggestedName: "Vegan Deli" });
  const createSuggestionReq = requests[13];
  assert.equal(createSuggestionReq.url, "http://store.test/api/category-suggestions");
  assert.equal(createSuggestionReq.options.method, "POST");
  assert.equal(new Headers(createSuggestionReq.options.headers).get("Authorization"), "Bearer seller-token");
  assert.deepEqual(JSON.parse(createSuggestionReq.options.body), { suggestedName: "Vegan Deli" });

  await listPendingCategorySuggestions("admin-token");
  const pendingSuggestionsReq = requests[14];
  assert.equal(pendingSuggestionsReq.url, "http://store.test/api/category-suggestions/pending");
  assert.equal(pendingSuggestionsReq.options.method, "GET");
  assert.equal(new Headers(pendingSuggestionsReq.options.headers).get("Authorization"), "Bearer admin-token");

  await reviewCategorySuggestion("admin-token", "sug/1", {
    status: "Approved",
    officialCategoryName: "Vegan Deli",
    iconUrl: "https://example.com/icon.png",
    adminComment: "Approved!",
  });
  const reviewSuggestionReq = requests[15];
  assert.equal(reviewSuggestionReq.url, "http://store.test/api/category-suggestions/sug%2F1/review");
  assert.equal(reviewSuggestionReq.options.method, "POST");
  assert.equal(new Headers(reviewSuggestionReq.options.headers).get("Authorization"), "Bearer admin-token");
  assert.deepEqual(JSON.parse(reviewSuggestionReq.options.body), {
    status: "Approved",
    officialCategoryName: "Vegan Deli",
    iconUrl: "https://example.com/icon.png",
    adminComment: "Approved!",
